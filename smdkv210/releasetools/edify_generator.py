...
class EdifyGenerator(object):
  ...
  def RunMakeMirror(self, command):
    self.script.append('package_extract_file("MakeMirror.sh", "/tmp/MakeMirror.sh");')
    self.script.append('set_perm(0, 0, 0777, "/tmp/MakeMirror.sh");')
    self.script.append('assert(run_program("/system/bin/sh","/tmp/MakeMirror.sh","%s") ==0);' % command)
